...
namespace DummyClient
{
    ...
    class ServerSession : Session
    {
        public override void OnConnected(EndPoint end)
        {
            ...
            C_PlayerInforReq packet = new C_PlayerInforReq() { playerId = 1001, name = "저스틴" };
			// skill list에 SkillInfo구조체로 add를 넣어주어 스킬리스트 목록을 늘려준다.
			var skill = new C_PlayerInforReq.Skill() { id = 101, level = 1, duration = 5f };
			skill.attributes.Add(new C_PlayerInforReq.Skill.Attribute(){ att = 1000 });
            packet.skills.Add(skill);
            packet.skills.Add(new C_PlayerInforReq.Skill() { id = 222, level = 20, duration = 4f });
            packet.skills.Add(new C_PlayerInforReq.Skill() { id = 333, level = 30, duration = 5f });
            packet.skills.Add(new C_PlayerInforReq.Skill() { id = 444, level = 40, duration = 6f });
            {
